...
const baseLogSchema = z.object({
  ...
  itemCategoryCustom: z.string().max(100, "Nama kategori kustom maksimal 100 karakter").optional().nullable(),
  imageUrl: z.string().url("Format image URL tidak valid").optional().nullable(),
  amount: z.coerce.number().min(0, "Jumlah harus lebih besar dari atau sama dengan 0"),
...
export const createLogSchema = baseLogSchema.superRefine((data, ctx) => {
  ...
  const hasCustom = data.itemCategoryCustom && data.itemCategoryCustom.trim() !== "";
  ...
  if (isOther && !hasCustom) {
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Kategori kustom hanya boleh diisi jika memilih kategori 'lainnya'",
      path: ["itemCategoryCustom"],
    });
  }
});

export const updateLogSchema = baseLogSchema.partial().superRefine((data, ctx) => {
